...
```mermaid
flowchart TB
    MOL["Molécula: c1ccccc1Cl"]
    FP["Morgan ECFP4 radius=2<br/>vector 2048 bits"]
    BITS["Bit 457: Cl en benceno<br/>Bit 892: anillo aromático 6<br/>Bit 1203: C aromático + Cl<br/>… mayoría en 0"]
...
```mermaid
flowchart LR
    IN["Fingerprint 2048"] --> L1["Linear 2048→512<br/>BN · ReLU · Dropout"]
    L1 --> L2["Linear 512→256<br/>BN · ReLU · Dropout"]
    L2 --> OUT["Linear 256→12<br/>logits multitarea"]
```
...
```mermaid
flowchart TB
    TXT["SMILES texto: c1ccccc1Cl"]
    EMB["Embedding 50 dims / carácter"]
    CNN["Conv1D 192 filtros, kernel=3"]
    GRU1["GRU bidireccional 224"]
    GRU2["GRU bidireccional 384"]
    CLS["Linear 768→12 logits"]

    TXT --> EMB --> CNN --> GRU1 --> GRU2 --> CLS
```
...
python scripts/fase2/train_baselines.py -v

# Ver distribución de etiquetas por tarea
python scripts/fase2/train_baselines.py --label-stats
```
...
## 5. Relación con el visor web (`viz/`)

Los baselines son modelos de **evaluación offline**: entrenan sobre `graphs_*.pt`, reportan AUC en `outputs/results/baseline_results.csv` y validan que el pipeline de datos funciona antes de entrenar la GNN.

El **GNN-Tox Viewer** (`viz/`) **no expone los baselines** al usuario final. La interfaz web y la API REST (`/api/predict`, `/api/analyze`) cargan exclusivamente el modelo GNN-GIN (`outputs/models/best_gin_model.pt`) porque:

- La hipótesis del proyecto exige que la GNN supere a los baselines en AUC
- Las explicaciones XAI (GNNExplainer, Grad-CAM) solo están definidas sobre la arquitectura GIN
- El visor está orientado a reguladores (MIDA/MINSA), no a comparación de modelos

La tabla comparativa RF / MLP / SMILES2vec vs GIN sigue siendo el entregable de esta fase; el visor es el entregable de comunicación de las Fases III–IV.

| Contexto | Baselines (Fase II) | Visor web (`viz/`) |
|---|---|---|
| Propósito | Referencia de rendimiento | Exploración interactiva |
| Representación | Fingerprints / SMILES texto | Grafo molecular (GIN) |
| Salida | CSV con AUC por tarea | Probabilidades + XAI en tiempo real |
| Requiere GPU | No (RF/MLP); opcional (S2V) | Opcional (acelera inferencia) |
...
| `scripts/fase2/train_baselines.py` | Script que entrena los 3 y guarda resultados |
